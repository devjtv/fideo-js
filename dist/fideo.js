...
const S = {
  mobile: 767,
  tablet: 1024
}, R = /* @__PURE__ */ new Set(["", "true", "1", "yes", "on"]), B = /* @__PURE__ */ new Set(["false", "0", "no", "off"]), I = "[data-fideo]", f = {
  play: !0,
  timeline: !0,
  currentTime: !0,
  duration: !0,
...
function Q(s, t = {}) {
  ...
  }, r = e.fideoProvider, a = t.provider ?? r ?? "auto", n = { ...q(s), ...t.sources }, d = a === "auto" ? G(s, n) : a, l = t.viewport ?? !1, c = h(e.fideoBackground, t.background ?? !1);
  return {
...
class X extends E {
  constructor(e, i) {
    ...
    const a = (n = window.Vimeo) != null && n.Player ? Promise.resolve() : F("https://player.vimeo.com/api/player.js");
    this.ready = a.then(() => {
      if (!this.destroyed)
  ...
  async setVolume(e) {
    var i;
    await this.ready, !this.destroyed && (await ((i = this.player) == null ? void 0 : i.setVolume(ee(e))), await this.sync("volumechange"));
  }
  async setMuted(e) {
    var i;
...
class te extends E {
  ...
  async setVolume(e) {
    await this.ready, !this.destroyed && (this.player && (this.player.volume = ie(e)), this.sync("volumechange"));
  }
  async setMuted(e) {
...
class le {
  constructor(t, e, i) {
    ...
    this.adapter = t, this.wrapper = e, this.icons = { ...de, ...i.icons }, this.element = p("div", "fideo__controls");
    const r = this.element.attachShadow({ mode: "open" }), a = document.createElement("style");
    a.textContent = ne, r.appendChild(a), this.playButton = this.button("fideo__button fideo__play", "Play", this.icons.play, "play-button"), this.muteButton = this.button("fideo__button fideo__mute", "Mute", this.icons.volume, "mute-button"), this.track = this.range("fideo__track", 0, 1e3, 1, "timeline"), this.volume = this.range("fideo__volume", 0, 1, 0.01, "volume-slider"), this.currentTime = p("span", "fideo__time"), this.currentTime.setAttribute("part", "current-time"), this.duration = p("span", "fideo__time"), this.duration.setAttribute("part", "duration"), this.speedMenu = this.createSpeedMenu(i.playbackRates), this.fullscreenButton = this.button("fideo__button", "Fullscreen", this.icons.fullscreen, "fullscreen-button");
...
class ce {
  constructor(t, e, i) {
    ...
    this.onDestroy = i, this.element = t, this.options = e, this.wrapper = this.wrapElement(t, e), this.configureElement(), this.adapter = oe(e.provider, t, e), this.applyResponsiveMedia(), e.controls && (this.controls = new le(this.adapter, this.wrapper, e)), this.bindAdapterEvents(), this.bindClickToToggle(), this.bindResponsiveMedia(), this.bindBackgroundCover(), this.bindViewportPlayback(), document.addEventListener("fullscreenchange", this.handleFullscreenChange), this.adapter.setVolume(e.volume).catch(() => {
  ...
  configureElement() {
    ...
    this.element instanceof HTMLVideoElement ? (this.element.controls = !1, this.element.loop = this.options.loop, this.element.muted = this.options.muted, this.element.playsInline = this.options.playsInline, this.element.setAttribute("playsinline", "")) : (this.element.allow = ue(this.element.allow, ["autoplay", "fullscreen", "picture-in-picture", "encrypted-media"]), this.element.setAttribute("allowfullscreen", ""));
  }
  bindAdapterEvents() {
...
function g(s, t = {}) {
  const e = k.get(s);
  if (e) return e;
  const i = Q(s, t), r = new ce(s, i, (a, n) => {
...
typeof window < "u" && (Object.assign(window, { Fideo: he, createFideo: pe, initFideo: x, mountFideo: g }), me() || document.addEventListener("DOMContentLoaded", () => {
