...
var $ = (s, t, e) => t in s ? U(s, t, { enumerable: !0, configurable: !0, writable: !0, value: e }) : s[t] = e;
var o = (s, t, e) => $(s, typeof t != "symbol" ? t + "" : t, e);
...
const P = {
  mobile: 767,
  tablet: 1024
}, R = /* @__PURE__ */ new Set(["", "true", "1", "yes", "on"]), F = /* @__PURE__ */ new Set(["false", "0", "no", "off"]), B = "[data-fideo]", y = {
  play: !0,
  timeline: !0,
  currentTime: !0,
  duration: !0,
...
function K(s, t = {}) {
  ...
  }, r = e.fideoProvider, a = t.provider ?? r ?? "auto", n = { ...Q(s), ...t.sources }, d = a === "auto" ? Y(s, n) : a, l = t.viewport ?? !1, c = h(e.fideoBackground, t.background ?? !1), u = d !== "html5";
  return {
    ...
    preload: W(e.fideoPreload, t.preload ?? "metadata"),
    controlVisibility: Z(s, t.controlVisibility),
    viewport: q(e.fideoViewport, l),
    viewportThreshold: b(e.fideoViewportThreshold, t.viewportThreshold ?? 0.35),
    volume: b(e.fideoVolume, t.volume ?? 1),
    playbackRates: G(e.fideoPlaybackRates, t.playbackRates ?? [0.5, 1, 1.25, 1.5, 2]),
    backgroundAspectRatio: D(e.fideoBackgroundAspectRatio ?? t.backgroundAspectRatio, 16 / 9),
    sources: n,
    posters: { ...J(s), ...t.posters },
    breakpoints: i,
    icons: t.icons ?? {},
...
function M(s, t, e = window.innerWidth) {
...
class te extends S {
  constructor(e, i) {
    ...
    this.options.autoplay && (r.autoplay = 1), this.options.muted && (r.muted = 1), this.options.loop && (r.loop = 1), this.options.background && (r.background = 1), this.element.src = _(T(this.element.src), r);
    const a = (n = window.Vimeo) != null && n.Player ? Promise.resolve() : I("https://player.vimeo.com/api/player.js");
    this.ready = a.then(() => {
      if (!this.destroyed)
  ...
  async setVolume(e) {
    var i;
    await this.ready, !this.destroyed && (await ((i = this.player) == null ? void 0 : i.setVolume(ie(e))), await this.sync("volumechange"));
  }
  async setMuted(e) {
    var i;
  ...
  async setSource(e) {
    var i;
    await this.ready, !this.destroyed && (await ((i = this.player) == null ? void 0 : i.loadVideo({ url: _(T(e), this.providerParams()) })), await this.sync());
  }
  destroy() {
    var e;
...
class se extends S {
  constructor(e, i) {
    ...
    this.options = i, this.element = e, this.options.muted && (this.state.muted = !0), this.mediaId = z(e.src);
  ...
  async setVolume(e) {
    await this.ready, !this.destroyed && (this.player && (this.player.volume = re(e)), this.sync("volumechange"));
  }
  async setMuted(e) {
...
class ae extends S {
  constructor(e, i) {
    ...
    const n = ee(this.element, "fideo-youtube");
...
function de(s, t, e) {
  ...
  return e.lazy ? new le(s, t, e) : N(s, t, e);
}
function N(s, t, e) {
  return s === "youtube" ? new ae(t, e) : s === "vimeo" ? new te(t, e) : new se(t, e);
}
class le extends EventTarget {
  ...
  async init() {
    ...
    const e = { ...this.options, lazy: !1 }, i = N(this.provider, this.element, e);
...
class pe {
  constructor(t, e, i) {
    ...
    this.adapter = t, this.wrapper = e, this.icons = { ...he, ...i.icons }, this.element = p("div", "fideo__controls");
    const r = this.element.attachShadow({ mode: "open" }), a = document.createElement("style");
    a.textContent = ue, r.appendChild(a), this.playButton = this.button("fideo__button fideo__play", "Play", this.icons.play, "play-button"), this.muteButton = this.button("fideo__button fideo__mute", "Mute", this.icons.volume, "mute-button"), this.track = this.range("fideo__track", 0, 1e3, 1, "timeline"), this.volume = this.range("fideo__volume", 0, 1, 0.01, "volume-slider"), this.currentTime = p("span", "fideo__time"), this.currentTime.setAttribute("part", "current-time"), this.duration = p("span", "fideo__time"), this.duration.setAttribute("part", "duration"), this.speedMenu = this.createSpeedMenu(i.playbackRates), this.fullscreenButton = this.button("fideo__button", "Fullscreen", this.icons.fullscreen, "fullscreen-button");
  ...
  changeVolume() {
    this.wrapper.classList.add("is-user-active");
    const t = this.adapter.getState(), e = V(Number(this.volume.value)), i = e === 0;
...
class me {
  constructor(t, e, i) {
    ...
    this.onDestroy = i, this.element = t, this.options = e, this.wrapper = this.wrapElement(t, e), this.configureElement(), this.adapter = de(e.provider, t, e), this.applyResponsiveMedia(), e.controls && (this.controls = new pe(this.adapter, this.wrapper, e)), this.bindAdapterEvents(), this.bindClickToToggle(), this.bindResponsiveMedia(), this.bindBackgroundCover(), this.bindViewportPlayback(), document.addEventListener("fullscreenchange", this.handleFullscreenChange), this.adapter.setVolume(e.volume).catch(() => {
  ...
  configureElement() {
    ...
    this.element instanceof HTMLVideoElement ? (this.element.controls = !1, this.element.loop = this.options.loop, this.element.muted = this.options.muted, this.element.playsInline = this.options.playsInline, this.element.preload = this.options.preload, this.element.setAttribute("playsinline", "")) : (this.element.allow = fe(this.element.allow, ["autoplay", "fullscreen", "picture-in-picture", "encrypted-media"]), this.element.setAttribute("allowfullscreen", ""));
  }
  bindAdapterEvents() {
  ...
  applyResponsiveMedia() {
    const t = M(this.options.posters, this.options.breakpoints);
    this.adapter.setPoster && this.adapter.setPoster(t ?? ""), this.applyPosterOverlay(t);
    const e = M(this.options.sources, this.options.breakpoints);
...
function g(s, t = {}) {
  const e = k.get(s);
  if (e) return e;
  const i = K(s, t), r = new me(s, i, (a, n) => {
...
typeof window < "u" && (Object.assign(window, { Fideo: ye, createFideo: ve, initFideo: L, mountFideo: g }), ge() || document.addEventListener("DOMContentLoaded", () => {
